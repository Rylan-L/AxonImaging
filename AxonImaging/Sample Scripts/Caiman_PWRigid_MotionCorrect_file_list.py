...
using the Caiman rigid PW motion correction. 
...
#whether to delete F-formatted mmap files or not after completion of the script
delete_mmap=True

#mouse ID or prefix
prefix='M382713'

data_folder = r"D:\2-photon data\ChAT-IRES-Cre; Ai162\180402-M377041\Airpuff\1"
...
offset_mov = int(np.min(movies_chained))
...
for files in mc.fname_tot_els:
    #to return to orginal pixel values, add back the offset that was subtracted to make the movie negative
    loaded=np.subtract(cm.load(files),-offset_mov)
